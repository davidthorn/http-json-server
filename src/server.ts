...
interface Array {
    contains(identiable: Identifiable): Boolean
}

function contains(identiable: Identifiable, items: Identifiable[]): Boolean {
 ...
 * @returns
 */
function Database(name: string, completion: () => void) {
    let writesPerCycle = 60
    let totalInFile = 0
    let items: Identifiable[] = []
    let waitingToWrite: Identifiable[] = []
    let lock = false;
    const dbFolder = './db'
     ...
     * @returns
     */
    const read = async (): Promise<Identifiable[]> => {
        if (lock) {
     ...
     */
    const write = (data: Identifiable[]): void => {
    ...
    const exists = async (id: string): Promise<Boolean> => {
        ...
        return Promise.resolve<Boolean>(s)
    }
